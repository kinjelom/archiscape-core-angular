...
export class TeamService {
  ...
  update(team: ITeam): Observable<EntityResponseType> {
    return this.http.put<ITeam>(`${this.resourceUrl}/${getTeamIdentifier(team) as string}`, team, { observe: 'response' });
  }

  partialUpdate(team: ITeam): Observable<EntityResponseType> {
    return this.http.patch<ITeam>(`${this.resourceUrl}/${getTeamIdentifier(team) as string}`, team, { observe: 'response' });
  }
