...
export class ProjectElementComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.projectElementService
      ...
      .subscribe({
        next: (res: HttpResponse<IProjectElement[]>) => {
  ...
  trackId(_index: number, item: IProjectElement): number {
    return item.id!;
  }

  delete(projectElement: IProjectElement): void {
    const modalRef = this.modalService.open(ProjectElementDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IProjectElement[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/project-element'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
